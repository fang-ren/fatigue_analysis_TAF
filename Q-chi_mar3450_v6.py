...
def drawCirc(i, j):
    """
    Given a known pixel indice (i,j), draw a red ellipse around that pixel 
    with radius of 0.1 and 0.02
    """
    th = np.arange(0, 2*np.pi, .02)
    q = 0.02 * np.cos(th) + Q[i][j]
    c = 0.1 * np.sin(th) + chi[i][j]
    return q, c
...
############ code starts here #############
###########################################
basefile_path = 'C:\\Users\\Fang Ren\\Desktop\\TimAllen_data\\after2_3000_'
file_index = 1

save_path = 'C:\\Users\\Fang Ren\\Desktop\\TimAllen_data\\Processed\\'
...
while (1):
    print 'waiting for image', basefile_path + str(file_index), '...'
    while not os.path.exists(basefile_path + str(file_index) + '.tif'):
        time.sleep(1)
    print 'processing image', basefile_path + str(file_index), '...'
    im = Image.open(basefile_path + str(file_index) + '.tif')
    imArray = np.array(im)
...
#plt.clim(0, np.max(imArray))
    ...
    plt.grid()
    plt.savefig(save_path + str(file_index) + '_Qchi')
    
    # generate column average image
    plt.figure(6)
    plt.title('Normalized Average')
    normIntAve, chiList = binNorm(Ipol, chi, 0.01, 0.06)
    plt.plot(normIntAve, chiList)
    plt.xlabel('Normalized Intensity by Local Average')
    plt.ylabel('chi')
    plt.savefig(save_path + str(file_index) + '_Normalized_Average')
    
    plt.close("all")

# https://github.com/CrakeNotSnowman/Python_Message/blob/master/sendMessage.py
    if np.nanmax(normIntAve) > 1.1:
        print 'sending alert...'
        fromaddr = 'hitpxrd@gmail.com'
        toaddrs1  = '2036688741@messaging.sprintpcs.com'
        toaddrs2  = '9093053090@tmomail.net'
        toaddrs3  = 'ren.fang.ren@gmail.com'
        msg = "\r\n".join([
        "From: hitpxrd@gmail.com, 9093053090@tmomail.net,ren.fang.ren@gmail.com"
        "To: ren.fang.ren@gmail.com",
        "Subject: ALERT",
        "",
