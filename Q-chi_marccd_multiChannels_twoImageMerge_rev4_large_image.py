...
def chiSum(Intensity, chi, step):
    ...
    count = np.bincount((chi[keep].ravel()*resol).astype(int), np.ones((3072,3072))[keep].ravel().astype(int))
    chiLen = len(IntSum)
...
def Qmask(Q1, hw1, Q, Qmask_status):
    if Qmask_status == 'off':
        return np.ones((3072,3072))
    if Qmask_status == 'on':
        QMask = np.ones((3072,3072))
...
############ code starts here #############
###########################################
folder_path = 'P:\\bl11-3\\May2016\\NiFe_scans\\NiFe-W4-db15\\'  
base_filename = 'NiFe-W4-db15_prelimscan_05191908_'
...
# fitting parameters
### changed x0, y0, d, tilt, and lambda from the "rev3" version: now using 170mm calibration values
###x0 = 1537.30216637
x0 = 1537.39226861
###y0 = 1547.90094844
y0 = 1549.77625088
...
# set up 3000X3000 X, Y grids for cropped image array
X = [i+601 for i in range(3072)]
Y = [i+601 for i in range(3072)]
X, Y = np.meshgrid(X, Y)

while (1):   
    print("\r")    
    print 'waiting for image', basefile_path + file_index(index+1) + '.tif...'
    print("\r")    
    while not os.path.exists(basefile_path + file_index(index+1) + '.tif'):
        time.sleep(1)
    print 'processing image '+ basefile_path + file_index(index) + '.tif' 
    print("\r")
    print("\r")
    im_A = Image.open(basefile_path + file_index(index) + '.tif')
    imArray_A = np.array(im_A)  # crop image to speed up
    ...
    Q = np.zeros((3072,3072))
    chi = np.zeros((3072,3072))
    Ipol = np.zeros((3072,3072))
    Q[:], chi[:] = Qchi(X, Y, x0, y0, d, Rot, tilt, lamda)
    #Ipol = polarCorr(imArray, Q, chi, lamda, PP)

    beamStop = np.ones((3072,3072))
    ...
    for i in range(len(chiList)):
        if IntAve_total[i] > 4.25:
            zinger += 1
    
    # https://github.com/CrakeNotSnowman/Python_Message/blob/master/sendMessage.py
    if zinger > 1:
        print 'Multiple hotspots were found in file ' + base_filename + file_index(index) + ', sending level-1 alert...'
        print("\r\n")
        ...
        "Subject: Multiple hotspot ALERT",
        "",
        "Multiple hotspots were found in file " + base_filename + file_index(index)
        ])
        ...
        print 'A hotspot was found in file ' + base_filename + file_index(index) + ', sending level-1 alert...'
        print("\r")
        fromaddr = 'hitpxrd@gmail.com'
        toaddrs1 = '5053663332@vzwpix.com'
        toaddrs2  = '5052214328@vzwpix.com'
        ...
        "",
        "A hotspot was found in file " + base_filename + file_index(index)
        ])
        username = 'hitpxrd@gmail.com'
        password = 'HiTpXRD2016'
        server = smtplib.SMTP('smtp.gmail.com:587')
