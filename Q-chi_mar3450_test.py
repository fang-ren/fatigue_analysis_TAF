...
def chiSum(Intensity, chi, step):
    ...
    that bin (count)
    """
    chiMin = np.nanmin(chi)    
    chi = chi + np.pi
    resol = 1/step    
    keep = np.where(Intensity != 0)
    IntSum = np.bincount((chi[keep].ravel()*resol).astype(int), Intensity[keep].ravel().astype(int))
    count = np.bincount((chi[keep].ravel()*resol).astype(int), np.ones((2050,2050))[keep].ravel().astype(int))
    chiLen = len(IntSum)
...
save_path = 'P:\\bl11-3\\May2016\\Processed\\'
...
X = [i+701 for i in range(2050)]
Y = [i+701 for i in range(2050)]
X, Y = np.meshgrid(X, Y)

while (1):
    print 'waiting for image', basefile_path + str(file_index)[2:], '...'
    while not os.path.exists(basefile_path + str(file_index)[2:] + '.tiff'):
        time.sleep(1)
    print 'processing image', basefile_path + str(file_index)[2:], '...'
    im = Image.open(basefile_path + str(file_index)[2:] + '.tiff')
    imArray = np.array(im)
    imArray = imArray[700:2750,700:2750,0]   # crop image to speed up
    im.close()

    # Calculate Q, chi, polarized corrected intensity(Ipol) arrays
    Q = np.zeros((2050,2050))
    chi = np.zeros((2050,2050))
    Ipol = np.zeros((2050,2050))
    Q[:], chi[:] = Qchi(X, Y, x0, y0, d, Rot, tilt, lamda)
    Ipol = polarCorr(imArray, Q, chi, lamda, PP)


    beamStop = np.ones((2050,2050))
...
# https://github.com/CrakeNotSnowman/Python_Message/blob/master/sendMessage.py
    if np.nanmax(normIntAve) > (1+criterion):
        print 'sending alert...'
